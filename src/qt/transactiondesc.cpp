...
QString TransactionDesc::FormatTxStatus(const CWalletTx& wtx)
{
    ...
    else
    {
        ...
        else if (nDepth < TransactionRecord::NumConfirmations)//modified by curecoin to adjust the confirm
...

    {
                    ...
                    if (wallet->IsMine(txout))
                    {
                        ...
                        if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address))
                        {
                            if (wallet->mapAddressBook.count(address))
                            {
                                ...
                                strHTML += "<b>" + tr("To") + ":</b> ";
                                strHTML += GUIUtil::HtmlEscape(CcurecoinAddress(address).ToString());
        ...
        if (wtx.IsCoinBase() && nCredit == 0)
        {
            ...
            if (wtx.IsInMainChain())
                strHTML += curecoinUnits::formatWithUnit(curecoinUnits::BTC, nUnmatured)+ " (" + tr("matures in %n more block(s)", "", wtx.GetBlocksToMaturity()) + ")";
            else
        ...
        else if (nNet > 0)
        {
            ...
            // Credit
            //
            strHTML += "<b>" + tr("Credit") + ":</b> " + curecoinUnits::formatWithUnit(curecoinUnits::BTC, nNet) + "<br>";
        }
        else
        {
            ...
            if (fAllFromMe)
            {
                ...
                BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                {
                    ...
                    strHTML += "<b>" + tr("Debit") + ":</b> " + curecoinUnits::formatWithUnit(curecoinUnits::BTC, -txout.nValue) + "<br>";
                }
                ...
                if (fAllToMe)
                {
                    ...
                    strHTML += "<b>" + tr("Debit") + ":</b> " + curecoinUnits::formatWithUnit(curecoinUnits::BTC, -nValue) + "<br>";
                    strHTML += "<b>" + tr("Credit") + ":</b> " + curecoinUnits::formatWithUnit(curecoinUnits::BTC, nValue) + "<br>";
                }

                int64 nTxFee = nDebit - wtx.GetValueOut();
                if (nTxFee > 0)
                    strHTML += "<b>" + tr("Transaction fee") + ":</b> " + curecoinUnits::formatWithUnit(curecoinUnits::BTC, -nTxFee) + "<br>";
            }
            else
            {
                //
        ...
        strHTML += "<b>" + tr("Net amount") + ":</b> " + curecoinUnits::formatWithUnit(curecoinUnits::BTC, nNet, true) + "<br>";
