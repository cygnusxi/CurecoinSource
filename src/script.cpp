...
bool Solver(const CScript& scriptPubKey, txnouttype& typeRet, std::vector<std::vector<unsigned char> >& vSolutionsRet)
{
    ...
    if (mTemplates.empty())
    {
        ...
        // curecoin address tx, sender provides hash of pubkey, receiver provides signature and pubkey
...
isminetype IsMine(const CKeyStore &keystore, const CScript& scriptPubKey)
{
    ...
    {
        CTxDestination dest;
        if (ExtractDestination(scriptPubKey, dest) && keystore.HaveWatchOnly(dest))
            return MINE_WATCH_ONLY;
        return MINE_NO;
    }
